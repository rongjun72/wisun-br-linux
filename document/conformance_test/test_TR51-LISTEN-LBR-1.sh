...
# -----------------------------------------------------------------------
# Test case :   Border Router as DUT [TR51-LISTEN-LBR-1]
# -----------------------------------------------------------------------
# Description:  
# Note:  This test case is identical to the test case 
# [DIRECT-MIXED-DWELL-LBR-1] except this test case makes use of the 
# TR51 Channel Function.
# This test case verifies the channel hop sequence required for Wi-SUN 
# FAN implementation using the TR51 Channel Function (value of 1).   
# The Regulatory Domain should be set to 0x1, Operating Class 2 
# (400 Khz channel spacing) and the data rate as 150kbps for 
# this section of tests..
# -----------------------------------------------------------------------
# The Test bed devices A-B and the LBR in the test are all configured for 
# the TR51 Channel Function using a dwell interval of 15ms.   
# The Test bed devices C-D are configured for the TR51 Channel Function 
# using a dwell interval of 255ms.  The Test bed devices E-H are 
# configured for the Direct Hash Channel Function using a dwell interval 
# of 15ms. The TBU’s will also use a Broadcast Interval (BI) 
# of 1020 ms, and a Broadcast Dwell Interval (BDI) of 255 ms.  
# The DUT is similarly configured for the TR51 Channel function 
...
# -----------------------------------------------------------------------
TEST_CASE_NAME="TR51-LISTEN-LBR-1"
...
time_checked=0;
packet_checked="";
step0_time=100; step1_time=150; step2_time=400;
...
if [ "$debug_option" = "DEBUG_ANALYSIS" ]; then
    ...
    sleep 5
    # change channel function of BR DUT to "TR51", meanwhile 0-255 allowed channels and 15ms UDI
    ssh ${BRRPI_usr}@${BRRPI_ip} "sudo wsbrd_cli set-fhss-uc-function 1 65535 15";
    sleep 20
