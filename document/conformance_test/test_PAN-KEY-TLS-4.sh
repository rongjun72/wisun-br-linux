...
# -----------------------------------------------------------------------
# Test case :   Border Router as DUT [PAN-KEY-TLS-4]
# -----------------------------------------------------------------------
# Description:  
# This test case verifies the ability of Border Router (DUT) to exchange 
# EAPOL frames with Joiner node to perform a 4-Way handshake and generate
# a pairwise transient key (PTK) and group transient Key.
# -----------------------------------------------------------------------
TEST_CASE_NAME="PAN-KEY-TLS-4"
...
# [PAN-KEY-TLS-4] Pass/Fail Criteria
# -------------------------------------------------------------------------------------------------
...
if [ -n $time_DUT_1st_4WH ]; then 
    ...
    steps_pass[13]=1;
    if [ "$eapol_keydes_key_info" != "0x008a" ];then steps_pass[13]=0; echo "----Step13 FAIL: Key Information: $eapol_keydes_key_info NOT 0x008a"; fi
    if [ $eapol_keydes_key_len -ne 16 ];        then steps_pass[13]=0; echo "----Step13 FAIL: Key Length NOT 16"; fi
    if [ $eapol_keydes_replay_counter -ne 0 ];  then steps_pass[13]=0; echo "----Step13 FAIL: Key Replay Counter NOT 0"; fi
    ...
    if [ $eapol_keydes_mic -ne 0 ];             then steps_pass[13]=0; echo "----Step13 FAIL: Key RIC NOT 0"; fi
    if [ $eapol_keydes_data_len -ne 22 ];       then steps_pass[13]=0; echo "----Step13 FAIL: Key Data Length  NOT 22"; fi
    
    if [ ${steps_pass[13]} -eq 1 ]; then echo "----Step13 PASS: Border Router DUT issues EAPOL-KEY to Joiner."; fi
else
    steps_pass[13]=0; echo "----Step13 FAIL: Border Router DUT does NOT issue EAPOL-KEY to Joiner."
fi

time_4WH_JOINER_SEND_PACKET=($(packet_receive_check ${LOG_PATH}/output_node.csv -t 3 $wsnode0_mac 4 $BRRPI_mac 5 "wpan:eapol" 6 "6" 7 "3"));
...
index_JOINER_1st_4WH=0;
for idx in $(seq 0 ${#time_4WH_JOINER_SEND_PACKET[@]}); do
    greate_test=$(echo "${time_4WH_JOINER_SEND_PACKET[$idx]} > $time_DUT_1st_4WH" | bc -l);
    if [ $greate_test -eq 1 ]; then
        time_JOINER_1st_4WH=${time_4WH_JOINER_SEND_PACKET[$idx]};
        index_JOINER_1st_4WH=$idx;
        echo "time_JOINER_1st_4WH: $time_JOINER_1st_4WH"
        break;
    else
        unset time_4WH_JOINER_SEND_PACKET[$idx];
    fi
done
echo "time_4WH_JOINER_SEND_PACKET: ${time_4WH_JOINER_SEND_PACKET[@]}"
...
if [ -n $time_JOINER_1st_4WH ]; then 
    ...
    steps_pass[14]=1;
    if [ "$eapol_keydes_key_info" != "0x010a" ];then steps_pass[14]=0; echo "----Step14 FAIL: Key Information: $eapol_keydes_key_info NOT 0x010a"; fi
    if [ $eapol_keydes_key_len -ne 0 ];         then steps_pass[14]=0; echo "----Step14 FAIL: Key Length NOT 0"; fi
    if [ $eapol_keydes_replay_counter -ne 0 ];  then steps_pass[14]=0; echo "----Step14 FAIL: Key Replay Counter NOT 0"; fi
...
if [ -n $time_DUT_2nd_4WH ]; then 
    ...
    eapol_keydes_key_len=${STEP15_DUT_ISSUE_PACKET[13]};
    eapol_keydes_replay_counter=${STEP15_DUT_ISSUE_PACKET[14]};
    eapol_keydes_nonce=${STEP15_DUT_ISSUE_PACKET[15]};
    eapol_keydes_key_iv=${STEP15_DUT_ISSUE_PACKET[16]};
    eapol_keydes_rsc=${STEP15_DUT_ISSUE_PACKET[17]};
