...
# -----------------------------------------------------------------------
# Test case :   Border Router as DUT [6LO-ND-LBR-1]
# -----------------------------------------------------------------------
# Description:  
# The 6LoWPAN ND test cases incorporate the Startup (Discovery) and 
# Security Bootstrapping and RPL Join/Route Establishment test procedures 
# and assume successful execution by the DUT (Border Router).  A test 
# bed device will then act as the Joiner and execute the RPL exchange 
# with the Border Router DUT to prompt establishment of its Neighbor 
# Cache Entry on the devices parents.
# -----------------------------------------------------------------------
...
#   b.Devices A joined to PAN A at Rank 1 in join state 5
#   c.Joiner Device E at Rank 2 in join state 1
# -----------------------------------------------------------------------
TEST_CASE_NAME="6LO-ND-LBR-1"
...
CSV_PACKET_FIELD_TABLE=(
 ...
 55: dhcpv6.iaaddr.ip                                56: icmpv6.nd.ns.target_address                                        
...
#---------- 6LoWPAN Neighbor Discovery ananlysis really start here
echo "";echo "---- 6LoWPAN Neighbor Discovery ananlysis begin ----------------"
#echo "-----------------"
STEP_PASSFAIL_Criteria=(
"output csv file:"  ${NodeCsvFile}                                                          
"Step number:"      "Step${#steps_pass[@]}"
"Step Description:" "Test bed device A sends “NS with ARO” to Border Router DUT (sent with AR=1)" 
"time range:"       $time_checked               $(echo "$time_checked + 130.000000" | bc -l)
"match items:"      "wpan.src64"                $wsnode0_mac
"match items:"      "wpan.dst64"                $BRRPI_mac
